    ...
    <div class="intro-y col-span-12 lg:overflow-visible">
        ...
        @foreach($carrier_posts as $carrier_post)
            ...
            <div @if($i == 0) @php $i = $i + 1; @endphp @else class=" md:mt-2" @endif>
                ...
                <span
                    class="absolute md:hidden right-50 top-10 -mt-4 -mr-4 ml-4 px-5 py-1 text-white rounded-full @if($carrier_post->status == 1) bg-theme-9 @elseif($carrier_post->status == 0) bg-theme-6 @endif rounded">@if($carrier_post->status == 1)
                        Свободно
                    @elseif($carrier_post->status == 0)
                        Зайнет
                    @endif</span>
                    ...
                    <div onclick="openCaption(2)" id="1"
                         class="grid grid-cols-12 md:hidden shadow-md bg-white rounded-lg p-5 mt-10 font-medium">
                        <div class="col-span-4 mt-4"><span
                                class="text-xl mt-6 font-bold text-theme-1">{{$carrier_post->price.' '.$carrier_post->currency['name_'.$locale]}}</span>
                        </div>
                        ...
                        <div class="col-span-4">
                        <span
                            class="text-gray-600">{{$carrier_post->from['name_'.$locale]}} - {{$carrier_post->to['name_'.$locale]}}</span>
                            <br>
                            <span
                                class="text-lg font-medium">{{date('d.m.Y', strtotime($carrier_post->departure_time))}}</span>
                        </div>
                        <div class="col-span-4">
                            <div class="w-fit mx-auto">
                            <span class="text-gray-600 whole-text">{{$carrier_post->delivery_type($carrier_post->user->id)['name_'.$locale]}}/
                                {{$carrier_post->vehicle_type($carrier_post->user->id)['name_'.$locale]}}</span>
                                <br class="whole-text">
                                <span>{{$carrier_post->trailer_size($carrier_post->user->id)->length.'m x '.$carrier_post->trailer_size($carrier_post->user->id)->width.'m x '.$carrier_post->trailer_size($carrier_post->user->id)->height.'m'}}</span>
                            </div>
                        </div>
                        <div class="col-span-4">
                            <div class="w-fit mx-auto">
                                <span class="text-gray-600">{{$carrier_post->free_weight}} t</span>
                                <br>
                                <span>Железо</span>
                            </div>
                        </div>
                        <div id="caption-2" class="col-span-12 text-left caption-hidden">
                            @if($carrier_post->description($carrier_post->id))
                                {{$carrier_post->description($carrier_post->id)->description}}
                            @else
                                <span class="text-danger">Description not found</span>
